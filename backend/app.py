...
class resume_analyzer:
    ...
    def openai(openai_api_key, chunks, analyze):
        ...
        # Directly pass chunks as input
        docs = "\n".join(chunks)  # Combine all text chunks for Gemini analysis

        # Use Gemini Pro Model
        model = genai.GenerativeModel("gemini-1.5-flash")

        # Process query and generate response
        response = model.generate_content(analyze)
    ...
    def resume_summary():
        ...
        with st.form(key='Summary'):
            ...
            with col1:
                openai_api_key = st.text_input(label='Enter Gemini API Key', type='password')
